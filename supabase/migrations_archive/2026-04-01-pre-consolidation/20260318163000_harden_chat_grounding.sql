...
WHERE
  (
    lower(coalesce(name, '')) ~ '(backend|rag|supabase|telemetria|telemetry|embedding|prompt|schema|vercel|api|migracao|migration|lovable|layout|motion)'
    OR lower(coalesce(file_name, '')) ~ '(backend|rag|supabase|telemetria|telemetry|embedding|prompt|schema|vercel|api|migracao|migration|lovable|layout|motion)'
  )
  AND coalesce(topic_scope, 'sei_rio') <> 'clara_internal';
...
BEGIN
  ...
  WITH semantic AS (
    ...
    WHERE dc.is_active = true
      ...
      AND COALESCE(d.topic_scope, 'sei_rio') <> 'clara_internal'
