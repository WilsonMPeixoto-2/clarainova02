...
BEGIN
  ...
  combined AS (
    SELECT
      ...
      (
        COALESCE((1.0::double precision / (rrf_k + s.rank_ix)::double precision), 0.0::double precision) +
        COALESCE((1.0::double precision / (rrf_k + k.rank_ix)::double precision), 0.0::double precision)
      ) AS rrf_score
    FROM semantic s
  ...
  SELECT
    ...
    combined.rrf_score::double precision AS similarity
  FROM combined
