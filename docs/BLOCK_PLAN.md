...
- Frente imediata mais importante: executar a trilha prioritária refinada `R4B-R6B` dentro do BLOCO 5 antes de retomar os subblocos canônicos `5B-5F`, agora já com `R0-R2` publicados, a regressão pós-publicação corrigida e `R3A-R4A` publicados em produção
...
- Próxima ação: abrir o `R4B` com dashboard admin das perguntas sem cobertura e dos gaps do corpus, reutilizando `query_analytics`, feedback do usuário e `rag_quality_score`
