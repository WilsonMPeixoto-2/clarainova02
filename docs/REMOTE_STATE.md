...
- `chat`
  - status: `ACTIVE`
  - versão observada: `33`
  - última atualização observada: `2026-04-05 13:32:33 UTC`
...
- Gemini / embeddings:
  ...
  - situação conhecida:
    ...
    - a produção web foi atualizada por promoção de preview `READY` após erro interno da Vercel no deploy manual direto, chegando ao pacote do repositório até o commit `dde134f`
...
- Corpus inicial:
  ...
  - situação conhecida:
    ...
    - o benchmark canônico remoto pós-publicação do fechamento `R6A-R6B` ficou green:
      - `Didático`: `16/16 HTTP 200`, `16/16 scopeExact`, `15/16 expectedAllMet`, `avgFinalConfidence 0.98`
      - `Direto`: `16/16 HTTP 200`, `16/16 scopeExact`, `15/16 expectedAllMet`, `avgFinalConfidence 0.98`
    - a próxima frente de corpus e retrieval volta agora para os subblocos `5B-5F`, além da substituição do Decreto `55.615` por texto íntegro oficial
